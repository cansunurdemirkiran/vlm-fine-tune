...
def preprocess_vilt(example, image_dir, processor):
...
def load_vilt_dataset(annotation_file, image_dir):
    processor = ViltProcessor.from_pretrained("dandelin/vilt-b32-mlm")
    dataset = load_dataset("json", data_files=annotation_file, field="annotations")["train"]

    dataset = dataset.map(lambda e: preprocess_vilt(e, image_dir, processor),
                          remove_columns=dataset.column_names,
                          batched=False)

    dataset.set_format(type="torch", columns=["pixel_values", "input_ids", "attention_mask"])
