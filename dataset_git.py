...
def preprocess_git(example, image_dir, processor):
...
def load_git_dataset(annotation_file, image_dir):
    processor = AutoProcessor.from_pretrained("microsoft/git-base")
    dataset = load_dataset("json", data_files=annotation_file, field="annotations")["train"]

    dataset = dataset.map(lambda e: preprocess_git(e, image_dir, processor),
